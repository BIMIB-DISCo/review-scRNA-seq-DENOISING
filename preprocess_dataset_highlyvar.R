#Normalize and filter datasets and select their top 500-1000-2000-5000 most variable genes. 
...
filterDataset <- function(counts, minCellPerc, minGenePerc, nVariable){
  ...
  tmp_so = CreateSeuratObject(counts = counts[idKeep,idCKeep])
  ...
  tmp_so <- FindVariableFeatures(tmp_so, 
                                 ...
                                 nfeatures = nVariable)
  idKeep = strtoi(VariableFeatures(tmp_so))
  ...
  print(paste0("Kept ",length(idKeep), " genes and ", length(idCKeep), " cells."))
  return(res)
}
...
dirCSV_true = "./final_data/csv_mostvar/true_filt/"
dirCSV_noise = "./final_data/csv_mostvar/noise_filt/"
dirCSV_scaled = "./final_data/csv_mostvar/noise_filt_normalized/"
...
for(GT_f in trueNames){
  ...
  for(N_f in noiseNames){
    ...
    rownames(observed_counts$counts) = c(1:nrow(observed_counts$counts))
    ...
    rownames(true_counts$counts) = c(1:nrow(true_counts$counts))
    true_counts$pop = true_counts$pop[observed_counts$cell_id]

    variable_subsets = c(500,1000,2000,5000)
    for (nVariable in variable_subsets) {
      ...
      noiseFile = paste0(dirCSV_noise, 
                         gsub(".rds", paste0("_",
                                             nVariable,
                                             "_genes_noise.csv"), N_f))
      logscaledFile = paste0(dirCSV_scaled, 
                             gsub(".rds", paste0("_",
                                                 nVariable,
                                                 "_genes_noise_logScaled.csv"), 
                                  N_f))
      if(
