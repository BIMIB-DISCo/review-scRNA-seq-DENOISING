...
setwd(working_dir)
outdir = paste0(working_dir, "final_noise/")
...
# Where the subsampled ground truth for UMI and nonUMI datasets will be stored
if (!dir.exists('final_GT/rdata')) {
  dir.create('final_GT/rdata', recursive=T)
}

ground_truth_list = list.files("ground_truth/rdata/", pattern = "^GT")

for (ground_truth_file in ground_truth_list) {
  ground_truth = readRDS(paste0("./ground_truth/rdata/", ground_truth_file))
  
  for (protocol in c("UMI", "nonUMI")) {
    sym_name = gsub("GT_|.rds", "", ground_truth_file)
    result_folder = paste0(outdir, protocol, "_", sym_name, "/")
    ...
    if (protocol=="UMI") {
      ...
      saveRDS(object = ground_truth_new,
              file=paste0("final_GT/rdata/GT_UMI_", sym_name, ".rds"))
    } else {
      set.seed(1)
      m = 0.1
      ...
      ground_truth_new = rndCellSempler(ground_truth, 
                                        id_sampled_cells)
      saveRDS(object = ground_truth_new,
              file=paste0("final_GT/rdata/GT_nonUMI_", sym_name, ".rds"))
    }
    load("gene_len.rdata")
